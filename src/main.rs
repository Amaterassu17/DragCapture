...
struct DragApp {
    ...
    //Da gestire
    hotkey_map: HashMap<u32, Vec<Code>>,
    hotkey_created: bool,
...
impl DragApp {
    pub fn new(cc: &CreationContext<'_>) -> Self {
        ...
        Self {
            ...
            hotkeys_strings : DragApp::load_hotkey_map().0,
            hotkey_ui_status: false,
            changing_hotkey: vec![false; 7],

            hotkey_map: DragApp::load_hotkey_map().1,
            hotkey_created: true,
            hotkey_manager: GlobalHotKeyManager::new().unwrap(),
        }
    }

    fn load_hotkey_map() -> (Vec<String>, HashMap<u32, Vec<Code>>) {
        //Load hotkey mapping from configuration file
        let mut hotkeys_strings: Vec<String> = Vec::new();
        let mut buf = String::new();
        File::open(HOTKEY_FILE).unwrap().read_to_string(&mut buf).unwrap();
        let mut index: u32  = 0;
        println!("{:?}", buf);
        let mut hotkey_map: HashMap<u32, Vec<Code>> = HashMap::new();
        buf.split("\n").for_each(|x| {
            hotkeys_strings.push(x.to_string());
            let mut split = x.split(" + ");
            let mut value: Vec<Code> = Vec::new();
            split.for_each(|y| {
                value.push(DragApp::string_to_code(y.to_string()));
            });
            hotkey_map.insert(index, value);
            index = index + 1;
            println!("{:?}", x);
        });
        println!("{:?}", hotkey_map);
    ...
    pub fn update_hotkey_map(&mut self, id: u32, codes_string: Vec<String>) -> () {

        let mut codes : Vec<Code> = Vec::new();
        for code in codes_string.iter() {
            codes.push(StringCodeWrap::new(code.clone()).into())
        }
        self.hotkey_map.insert(id, codes);
        let mut string_file: Vec<String> = Vec::new();
        ...
        for code in self.hotkey_map.values() {
